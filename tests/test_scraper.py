...
class TestScraper:
    ...
    async def test_returns_article_content(self, scraper):
        with patch("bcn.common.scraper.async_playwright") as mock_pw_start:
            ...
            mock_el = AsyncMock()
            mock_el.inner_text = AsyncMock(return_value="This is the article body with enough content.")
    ...
    async def test_falls_back_to_body(self, scraper):
        with patch("bcn.common.scraper.async_playwright") as mock_pw_start:
            ...
            mock_el_long.inner_text = AsyncMock(return_value="Fallback body content that is long enough.")
            
            # The selectors are ["article", ".markdown-body", "main", "body"]
            # Return short for the first three, long for the last
            mock_page.query_selector = AsyncMock(side_effect=[mock_el_short, mock_el_short, mock_el_short, mock_el_long])
